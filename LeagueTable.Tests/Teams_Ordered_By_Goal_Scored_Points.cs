...
{
    public class Teams_Ordered_By_Goal_Scored_Within_Points
    {
        const int redTeam = 1;
        const int blueTeam = 2;
        const int greenTeam = 3;

        public Teams_Ordered_By_Goal_Scored_Within_Points()
        {
        }
        ...
        public void And_Played_Once_Each_Team_Once_Red_Team_Finishes_First(List<Result> matchResults)
        {
        ...
        [Theory]
        [ClassData(typeof(TestResultGenerator))]
        public void And_Played_Once_Each_Team_Once_Green_Team_Finishes_Last(List<Result> matchResults)
        {
            var sut = new TableCalculator();
        ...
        private class TestResultGenerator : IEnumerable<object[]>
        {
            ...
            {
                var redWin = new ResultBuilder().RegulationWin(redTeam, 2, blueTeam, 1).Build();
                var blueWin = new ResultBuilder().RegulationWin(blueTeam, 1, greenTeam, 0).Build();
